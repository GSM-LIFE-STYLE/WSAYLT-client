...
function Login() {
  ...
  return <div className="login">
    ...
    <div>
      ...
      <input className={`pw ${pw && 'status'} ${pwinco && 'incorrect'}`} onChange={e => setPw(e.target.value)} placeholder="비밀번호를 입력해주세요." type={(visible1 && 'password').toString()} />
      ...
      {pwinco && <span style={{ color: 'red' }}>비밀번호를 입력해주세요.</span>}
      <button onClick={e => { //비밀 번호 맞는지 확인
        pw ? setPwinco(false) : setPwinco(true);
